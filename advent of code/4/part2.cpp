...
int main(){
    ...
    for(unsigned int i=0; i<numeriChiamati.size();i++){
    //    std::cout<<numeriChiamati[i]<<" ";
    }
    int numberend = -1;
    ...
    for(unsigned int k=0; k<numeriChiamati.size();k++){
        ...
        for(it = celle.begin(); it!=celle.end(); ++it){
            for(unsigned int i=0; i<5;i++){
                for(unsigned int j=0;j<5;j++){
                    if((*it)[i][j]==numberCalled){
                        bool matrixWinner=false;
                        bool matrixRowsWinner = true;
                        bool matrixColumnWinner = true;
                        for(unsigned int a=0; a<5;a++){
                            for(unsigned int b=0;b<5;b++){
                                bool matrixRowsWinner= matrixRowsWinner && ((*it)[a][b]==-1);
                            }
                            matrixWinner = matrixWinner || matrixRowsWinner;
                        }
                        for(unsigned int a=0; a<5;a++){
                            for(unsigned int b=0;b<5;b++){
                                bool matrixColumnWinner= matrixColumnWinner && ((*it)[b][a]==-1);
                            }
                            matrixWinner = matrixWinner || matrixColumnWinner;
                        }
                        //std::cout<<matrixWinner;
                        if(matrixWinner==false){
                            (*it)[i][j]=-1;
